...
Likely misfire: a sub-second number from a prompt that hides work in a warm cache nobody else would have; a patch that changes answers; claiming a model judges better than the data shows; running a second model beside another GPU workload after the kernel panic.
