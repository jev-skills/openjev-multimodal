...
每个问题都要对完整提示做一次预填充。Qwen3.5 与 Qwen3.6 是混合架构，循环层无法回滚到任意缓存位置，所以 llama.cpp 过去会为每个问题重读共享 state、重新编码图片。现在多问题请求只计算一次共享前缀；llama.cpp 在此建立检查点，每个问题从这里继续，只处理自己的文本。一张截图的四个问题，耗时降到一半以下。判断结果不变；前缀单独成批计算，概率最多相差 0.067。
