...
**一个完整的俄罗斯方块游戏，由本地模型通过 SystemOne API 来玩。** 规则交给代码：列出所有合法走法、模拟结果、按下按键。判断交给模型：阅读简短的方案列表，只用 1 个输出 token 选出方案。
...
4. **展示结果。** 默认的 `vision` 提示用一张图片把每个方案执行后的棋盘画成带字母的小图，`+N` 表示消除的行数，空洞以深色空格显示。
...
- `vision`（默认）：文字事实加结果缩略图。小模型需要这张图。
- `text`：同样的事实，不发图片。
- `compact`：规则作为不变的 state，问题中只放当前方块和每个方案一行简短事实。API 会缓存 state，每次调用只读取 75–100 个新 token。大模型这样下得一样好，速度快数倍。

[设计实验](report/README.zh-CN.md#设计实验)在同样的 48 个状态上比较了 9 种呈现方式。决定结果的是精确的文字事实：对任何模型，只给图片都不够；整张棋盘截图比结果缩略图更慢也更差。

## 结果

每个档位、每种提示方式 5 个种子 × 100 个方块。完整数据、图表与视频见[测试报告](report/README.zh-CN.md)。

| 档位 | 模型 | 消除行数 · 视觉 | 消除行数 · 精简 | 调用 · 视觉 | 调用 · 精简 |
| --- | --- | ---: | ---: | ---: | ---: |
| `fast` | Qwen3.5-0.8B | 26.6 | 17.4 | 0.15 秒 | 0.03 秒 |
| `balanced` | Qwen3.5-4B | 35.8 | 33.0 | 0.65 秒 | 0.12 秒 |
| `quality` | Qwen3.6-35B-A3B | 38.2 | 38.2 | 0.91 秒 | 0.14 秒 |
| `max` | Qwen3.8-27B | 38.4 | 38.2 | 4.08 秒 | 0.73 秒 |

没有模型时，在同样的方案中随机选择，每局平均只消除 14.3 行（`play.py baseline`）。
...
uv run python examples/tetris/play.py baseline        # 无需模型
...
| `play.py` | `play`、`bench`、`ablation`、`baseline`、`probe` 命令 |
